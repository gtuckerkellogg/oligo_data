...
class SSSFile:
    ...
    def __init__(self, fname, raw_seq_data=None, sssseqs=None, src_data=None):
        ...
        self.name = fname
        self.sources_id = {} if not src_data else src_data  # if provided, it's a dictionary indexed by source id (i.e. 'source_<some id>')
        self.sources_path = invert_dict(self.sources_id)

        # store SSSSeq objects in list of provided, else start with empty list
        self.seqs = list(sssseqs) if sssseqs else []
...
class SSSSubSeq:
    ...
    def __init__(self, seq, order, start, description, seq_id, src_id="source_None", src_path=None):
        ...
        self.subseq_sequence = seq
        self.subseq_order = order
        self.subseq_start = start
        self.subseq_description = description
        self.subseq_id = seq_id
        self.subseq_src_id = src_id
